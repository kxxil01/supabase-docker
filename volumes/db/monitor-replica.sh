...
while true; do
    ...
    if [ -n "$LAG" ] && [ "$LAG" != "ERROR" ]; then
        ...
        RECOVERY=$(psql -h localhost -U postgres -d postgres -t -c "SELECT pg_is_in_recovery();" 2>/dev/null | tr -d ' ')
        if [ "$RECOVERY" = "t" ]; then
    ...
    REPLICATION_INFO=$(psql -h localhost -U postgres -d postgres -t -c "SELECT conninfo FROM pg_stat_wal_receiver;" 2>/dev/null | tr -d ' ')
